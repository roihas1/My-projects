...
async createSingleEliminationBracket(
  ...
  try {
    const registrations = await this.registrationService.getRegistrations(eventId, user);
    if (registrations.length < 2) {
      this.logger.error(`Insufficient teams registered for event ID "${eventId}".`);
      throw new Error(`Insufficient teams registered for event ID "${eventId}".`);
    }
    ...
    for (let round = 0; round < numberOfRounds; round++) {
      ...
      for (let i = 0; i < numMatchesInRound; i++) {
        ...
        schedule.date = this.calculateMatchTime(currentDate,startTime, i*15);
    ...
    this.logger.verbose(`Schedule created and saved successfully for event ID "${eventId}".`);
  } catch (error) {
    this.logger.error(
...
private calculateMatchTime(currentDate: Date, startTime: string, additionalMintutes: number): Date {
...
private async propagateWinnerToNextRound(
  ...
  let nextMatch = await this.scheduleRepository.findOne({
    where: [
      { round: nextRound, event: { id: eventId }, matchNumber: targetMatchNumber },
    ],
  ...
  if (!nextMatch) {
   ...
   if (nextMatch.team1 && nextMatch.team1.id === winner.id) {
    ...
    this.logger.verbose(`Winner team ID "${winner.id}" is already in the next match ID "${nextMatch.id}" as team1.`);
    return;
  } else if (nextMatch.team2 && nextMatch.team2.id === winner.id) {
   
    this.logger.verbose(`Winner team ID "${winner.id}" is already in the next match ID "${nextMatch.id}" as team2.`);
    return;
  }
  
  if (nextMatch.team1 && (nextMatch.team1.id == match.team1.id || nextMatch.team1.id == match.team2.id )) {
    nextMatch.team1 = winner;
    this.logger.verbose(`Replaced team1 with winner team ID "${winner.id}" in the next match ID "${nextMatch.id}".`);
  } else if (nextMatch.team2 &&(nextMatch.team2.id == match.team1.id || nextMatch.team2.id == match.team2.id )) {
    nextMatch.team2 = winner;
    this.logger.verbose(`Replaced team2 with winner team ID "${winner.id}" in the next match ID "${nextMatch.id}".`);
  } else {
    if (!nextMatch.team1) {
      nextMatch.team1 = winner;
...
private checkWinner(match: Schedule): Team | null {
  ...
  deleteAllEventGames(eventId: string, user: User): Promise<void> {
